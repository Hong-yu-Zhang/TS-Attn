...
class WanTransformerBlock(nn.Module):
    ...
    # ) -> torch.Tensor:
    #     """
    #     Removes isolated True values in a boolean mask by applying k×k erosion per frame.

    #     Args:
    #         mask_1d : [B, N_video]  (bool) - Input mask, flattened.
    #         T, H, W : int - Dimensions of the video (time, height, width).
    #         k : int - Kernel size for erosion.

    #     Returns:
    #         mask_eroded : [B, N_video]  (bool) - Mask after erosion, flattened.
    #     """
    #     B, N = mask_1d.shape
    #     assert N == T * H * W, "T·H·W 与 N_video 不匹配"

    #     # Reshape mask to [B, T, H, W] and add channel dimension
    #     grid = (~mask_1d).view(B, T, H, W).unsqueeze(2).float()  # [B, T, 1, H, W]

    #     grid = grid.view(B * T, 1, H, W)

    #     # Simulate min pooling using max pooling on inverted mask
    #     eroded = ~(F.max_pool2d(grid, kernel_size=k, stride=1, padding=k // 2).bool())

    #     eroded = eroded.view(B, T, H, W)

    #     # Remove channel dimension, flatten back to [B, N_video]
    #     mask_eroded = eroded.squeeze(2).view(B, N).bool()
    #     return mask_eroded
    def erode_isolated(
        self,
        mask_1d: torch.Tensor,
        ...
        """
        Removes isolated True values in a boolean mask by applying k×k erosion per frame,
        excluding the center point when checking the neighborhood.
        ...
        # Reshape mask to [B, T, H, W]
        grid = mask_1d.view(B, T, H, W).float()  # [B, T, H, W]

        # Reshape to [B×T, 1, H, W] for independent frame-wise operation
        grid = grid.view(B * T, 1, H, W)
    ...
    def forward(
        ...
        USE_ENV_CONTROL = (event_mask is not None) and (timestep[-1] in model_configs["Env_args"]["time_step"]) and use_control
